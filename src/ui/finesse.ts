...
import { calculateGhostPosition, createEmptyBoard, tryMove } from '../core/board';
...
export class BasicFinesseRenderer implements FinesseRenderer {
  ...
  render(gameState: GameState, visualization: FinesseVisualization): void {
    ...
    if (visualization.targetPosition && guidance?.visual?.highlightTarget !== false) {
    ...
    if (visualization.optimalSequence && (guidance?.visual?.showPath ?? false)) {
      this.renderSequenceOverlay(visualization.optimalSequence, visualization.currentStep || 0);
    }
  ...
  private renderPathStep(piece: ActivePiece, stepNumber: number, action: string): void {
    if (!this.ctx) return;
  ...
  private drawTargetCell(x: number, y: number, fillColor: string, borderColor: string): void {
    if (!this.ctx) return;
    ...
    this.ctx.strokeRect(pixelX + 1.5, pixelY + 1.5, this.cellSize - 3, this.cellSize - 3);
  ...
  private drawPathStepCell(x: number, y: number, color: string, stepNumber: number, action: string): void {
    if (!this.ctx) return;
  ...
  private drawActionIndicator(centerX: number, centerY: number, action: string): void {
    if (!this.ctx) return;
  ...
  private drawArrow(centerX: number, centerY: number, size: number, direction: 'left' | 'right' | 'down' = 'right'): void {
    if (!this.ctx) return;
  ...
  private drawRotationIndicator(centerX: number, centerY: number, radius: number, direction: 'cw' | 'ccw' = 'cw'): void {
    if (!this.ctx) return;
...
export function createFinesseVisualization(gameState: GameState): FinesseVisualization {
  ...
  // In guided mode, show target position and optimal path
  {
    ...
    if (gameState.active && guidance && guidance.target) {
      const targetInfo = { targetX: guidance.target.x, targetRot: guidance.target.rot } as const;
        // Calculate target Y position against current board
        const targetY = calculateDropPosition(gameState, gameState.active.id, targetInfo.targetX, targetInfo.targetRot);
        ...
        try {
          ...
          if (optimalSequences.length > 0) {
            ...
            if (optimalSequence) {
              ...
              visualization.pathTrace = generatePathTrace(spawnFromActive(gameState.active), optimalSequence);
