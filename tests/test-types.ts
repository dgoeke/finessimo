import type { GameState, Action, PieceId, ActivePiece, Board, Rot } from '../src/state/types';
...
export type MalformedAction = 
  | { type: 'Init'; seed?: unknown; timing?: unknown; gameplay?: unknown; mode?: unknown }
...
export type InvalidPiece = Partial<ActivePiece> | {
  id: unknown;
  rot: unknown;
  x: unknown;
  y: unknown;
...
export function isValidAction(action: unknown): action is Action {
  ...
  return typeof a.type === 'string' && [
    'Init', 'Tick', 'Spawn', 'Move', 'SoftDrop', 'Rotate', 
    'HardDrop', 'Hold', 'Lock', 'StartLockDelay', 'CancelLockDelay',
    'StartLineClear', 'CompleteLineClear', 'ClearLines', 'EnqueueInput'
  ].includes(a.type);
}

export function isValidPieceId(id: unknown): id is PieceId {
  return typeof id === 'string' && ['I', 'O', 'T', 'S', 'Z', 'J', 'L'].includes(id);
}

export function isValidRot(rot: unknown): rot is Rot {
  return typeof rot === 'string' && ['spawn', 'right', 'two', 'left'].includes(rot);
}

// Test-specific invalid action creators
export function createInvalidAction(overrides: Partial<MalformedAction>): MalformedAction {
  return {
    type: 'InvalidAction',
    ...overrides
  } as MalformedAction;
...
export function createPartialGameState(overrides: InvalidGameState): InvalidGameState {
  return overrides;
}

export function createCorruptedRng(seed: string, corruption: 'empty' | 'undefined' | 'null' = 'undefined'): CorruptedRng {
  const bag: (PieceId | undefined | null)[] = 
    corruption === 'empty' ? [] :
    corruption === 'undefined' ? [undefined] :
    [null];
