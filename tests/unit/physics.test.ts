...
describe('physics system', () => {
  describe('gravity', () => {
    ...
    it('should auto-lock piece after lock delay expires', () => {
      ...
      const newState = reducer(state, { type: 'Tick', timestampMs: expiredTime });
  ...
  describe('soft drop', () => {
    ...
    it('should use faster gravity when soft dropping', () => {
      ...
      const expectedInterval = Math.floor(1000 / state.timing.softDropCps);
  ...
  describe('lock delay mechanics', () => {
    it('should cancel lock delay on move', () => {
      let state = createStateWithPiece();
      state = { ...state, physics: { ...state.physics, lockDelayStartTime: 1000 } };
    ...
    it('should start lock delay manually', () => {
      ...
      const newState = reducer(state, { type: 'StartLockDelay', timestampMs: timestamp });
