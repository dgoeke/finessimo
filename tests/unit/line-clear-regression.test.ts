...
function createStateWithCompleteLine(lineClearDelayMs: number): GameState {
  ...
  if (state.active) {
    ...
    state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=4 -> cells at 4,5,6,7
    state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=5 -> cells at 5,6,7,8
    state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=6 -> cells at 6,7,8,9
    state = reducer(state, {
      type: "HardDrop",
  ...
  describe("Edge cases", () => {
    ...
    it("should handle multiple line clears with delay", () => {
      ...
      if (state.active) {
        ...
        state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=4
        state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=5
        state = reducer(state, { type: "Move", dir: 1, source: "tap" }); // x=6
        state = reducer(state, {
          type: "HardDrop",
          timestampMs: createTimestamp(1000),
        });
