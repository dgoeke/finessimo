...
describe("DOMInputHandler keybindings and suppression", () => {
  ...
  test("setKeyBindings persists and affects mapping", () => {
    ...
    const keyBindings =
      typeof parsed === "object" && parsed && "keyBindings" in parsed
    ...
    expect(
      dispatched.some(
        (a) => a.type === "EnqueueInput" && a.event.action === "LeftDown",
      ),
    ).toBe(true);
    ...
    expect(dispatched.some((a) => a.type === "Rotate" && a.dir === "CW")).toBe(
      true,
    );
  });
  ...
  test("DAS then ARR pulses using functional now param", () => {
    const state = {
      timing: { dasMs: 100, arrMs: 50, softDrop: 10, gravityMs: 1000 },
    ...
    expect(
      dispatched.find((a) => a.type === "Move" && a.source === "das"),
    ).toBeFalsy();
    ...
    const countBefore = dispatched.filter(
      (a) => a.type === "Move" && a.source === "das",
    ).length;
    handler.update(state, 1150);
    const countAfter = dispatched.filter(
      (a) => a.type === "Move" && a.source === "das",
    ).length;
