...
## Code Navigation & Refactor Workflow (no grep)
Use IDE-grade language features and compiler diagnostics for code understanding, navigation, and refactoring. Prefer these tools over plain text search for accuracy and safety.

**Rules (must follow)**
1. **Default to language tools** for “where is X?”, “who uses X?”, “what type is X?”, and “why is this error?”. Grep/plain text search is a last resort; explain why you fell back if you do.
2. **Diagnose before changing**: surface diagnostics on impacted files; use the file/line/column from diagnostics to drive jumps to definition/references and type hovers.
3. **Navigate, then edit**: locate exact symbols via definition/references; confirm types with hover; propose a **Diff Plan**; then apply minimal, line-scoped edits.
4. **Refactor safely**: use language-tool renames for symbol refactors; never manual string edits when a symbol rename is intended.
5. **Re-check after edits**: re-run diagnostics on touched files; if renames occurred, re-run references to catch stragglers.
6. **Show your work**: include a short **Tool Trace** (which tools/commands you used, with file/line/symbol) and a **Diff Plan** before applying edits.

**Navigation workflow**
1) Run diagnostics on failing/changed files  
2) Jump to definition; inspect types/nullability with hover  
3) List references to map blast radius (call sites, tests)  
4) **Diff Plan**: file+line targets, minimal changes, side-effect boundaries  
5) Apply edits: surgical, reversible changes only  
6) Re-check diagnostics (and references if rename) → verify green

**Response format (always)**
- **Summary** — one sentence goal/outcome  
- **Tool Trace** — bullet list of key tool calls/commands with params  
- **Analysis** — what types/defs/refs revealed  
- **Diff Plan** — file-and-line scoped plan  
- **Edits** — applied patch snippet(s)  
- **Post-check** — results from diagnostics; follow-ups/tests to run

> **Example (mini)**
> - Tool Trace:  
>   - `diagnostics("src/rng.ts") → error TS2322 at 40:6`  
>   - `definition("shuffle") → src/utils/array.ts:12`  
>   - `hover(src/utils/array.ts:12:18) → type T`  
>   - `references("shuffle") → tests + 2 call sites`  
> - Diff Plan: Narrow generic constraint and guard undefined before swap.  
> - Edits: `src/utils/array.ts` lines 8–20  
